...
_SI5351_CRYSTAL_FREQUENCY = 27000000.0  # Fixed 25mhz crystal on board.
...
class SI5351:
    ...
    class _PLL:
        ...
        @staticmethod
        def check_fvco(fvco):
            """Verify that PLL's fvco in in allowed range."""
            if not (600e6 <= fvco <= 900e6):
                raise ValueError("PLL's fvco out of range: {:3.2f}MHz (600MHz-900MHz)".format(
                    fvco/1e6
                ))
    ...
    class _Clock:
        ...
        def configure_integer(self, pll, divider):
            ...
            p2 = 0
            p3 = 1
            print("p1: {:d} / p2: {:d} / p3: {:d}".format(p1, p2, p3))
            self._configure_registers(p1, p2, p3)
    ...
    def _read_u8(self, address):
        ...
        data = self._i2c.readfrom_mem(self._i2c_address, address, 1)
        return data[0]
    ...
    def _write_u8(self, address, val):
        ...
        print("write: addr: {:02x} value: {:02x}".format(address & 0xff, val & 0xff))
